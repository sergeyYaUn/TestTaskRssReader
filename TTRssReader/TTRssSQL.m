...
static NSString * const TTRssDatabaseName = @"rssDb.sql3";
...
- (BOOL)saveRssNewsWithName:(NSString*)sourceName fromArray:(NSArray<NSDictionary*>*)array
{
    if([self openDatabaseWithName:TTRssDatabaseName]){
        ...
        for (NSDictionary * item in array){
            const char * insertNewsQuery = [[NSString stringWithFormat:@"INSERT INTO newstable (source, title, detail) VALUES ('%@', '%@', '%@')", sourceName, item[@"title"], item[@"detail"]] UTF8String];
